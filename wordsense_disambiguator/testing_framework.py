...
def main():
	...
	for language in ['English','Spanish']:
		for features in ['stp_stemmed_punct']:
			...
			for window in [20,25]:
				...
				for model in ['svm','nb']:
					for n in range(1200,1700,200):
						topn = {'method':'rel','n':n}
						#for method in [False,'pmi','rel']:
						#topn['method'] = method
						scores = score_params(train_vecs,test_vecs,language,
								      features,model,topn=topn,pca=False,
								      knn=5,window=window)
						print(scores)
...
# build context vectors based on window size and feature extraction method
def get_feature_vecs(language,dataset='train',features='standard',k=10):
	...
	feature_lookup = {'standard'   :standard,
			  ...
			  'stp_stemmed_punct':stp_stemmed_punct}
...
import string
def stp_stemmed_punct(context,idx,language):
	stemmer         = SnowballStemmer(language.lower())	
	exclude         = set(string.punctuation)
	s 		= context.childNodes[idx].wholeText.lower()
...
def write_results(results,language,model,features,topn,pca,k,window):
	if model=='knn': model='knn_{}'.format(k)
	if topn: topn = 'topn_{}_{}'.format(topn.get('method'),topn.get('n'))
	file_name = '{}-{}-{}-pca_{}-weightsT-window_{}.{}'.format(language,features,topn,pca,window,model)
...
#<hack>
def score_results(model_file,language,sense_map = ''):

	key_file = 'data/{}-dev.key'.format(language)
