...
class BerkeleyAligner():
	...
	def train(self, align_sents, num_iters):
		...
		def get_counts(align_sents,align):
		...
		# Initialize data structres and probability estimates
		...
		for i in range(0, num_iters):
			count_ef,total_f,count_align_ef,total_align_f = get_counts(align_sents,align_en)
			count_fe,total_e,count_align_fe,total_align_e = get_counts(inverted,align_fr)
			...
			for f in fr_vocab:
				for e in en_vocab:
					...
					total_f[f] 
					total_e[e]
					t[e][f] = ( count_ef.get(e).get(f) + count_fe.get(f).get(e) ) / ( total_f.get(f) + total_e.get(e) )
